...
function getEnumOptions(field: any) {
  ...
  if (!Array.isArray(values)) return [] as Array<{ label: string; value: any }>;
  return values.map((item: any) => {
    if (item && typeof item === 'object' && !Array.isArray(item)) {
      return { label: String(item.label ?? item.value ?? ''), value: item.value ?? item.label };
    }
...
async function openEnumEditor(options: QuickEditOptions, field: any, cell: HTMLElement) {
  const enumOptions = getEnumOptions(field);
  if (!enumOptions.length) return false;

  const multiple = ['multipleSelect', 'checkboxGroup'].includes(field?.interface);
  const select = document.createElement('select');
  select.dataset.ningInlineEditor = 'true';
  select.multiple = multiple;
  select.size = multiple ? Math.min(Math.max(enumOptions.length, 3), 7) : 1;
  const original = options.record?.[options.fieldPath];
  const originalValues = asArray(original);
  enumOptions.forEach((item, index) => {
    const option = document.createElement('option');
    option.value = String(index);
    option.textContent = item.label;
    option.selected = originalValues.some((value) => Object.is(value, item.value));
  ...
  const updatePosition = () => {
    ...
    select.style.width = `${Math.min(360, window.innerWidth - left - 16)}px`;
  };
  select.style.cssText =
    'position:fixed;box-sizing:border-box;border:1px solid #1677ff;border-radius:6px;padding:4px 8px;outline:none;background:#fff;z-index:1100;box-shadow:0 6px 16px rgba(0,0,0,.15);';
  if (multiple) select.style.maxHeight = '220px';
  ...
  const save = async () => {
    if (finished) return;
    const selected = Array.from(select.selectedOptions).map((item) => enumOptions[Number(item.value)]?.value);
    const value = multiple ? selected : selected[0] ?? null;
    if (JSON.stringify(value) === JSON.stringify(original)) return close();
  ...
  select.addEventListener('change', () => {
    if (!multiple) void save();
  });
  select.addEventListener('blur', () => void save(), { once: true });
...
async function openInlineEditor(options: QuickEditOptions) {
  ...
  if (isEnumField(field) && (await openEnumEditor(options, field, cell))) return;
  ...
  if (!multiline) {
    ...
    input.dataset.ningPositionCleanup = 'true';
